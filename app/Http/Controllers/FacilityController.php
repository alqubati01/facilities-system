...
class FacilityController extends Controller
{
  ...
  public function index(Request $request)
  {
    ...
    $facilities = Facility::with('branch', 'unit', 'currency', 'status')
    ...
    return view('content.facilities.index', [
  ...
  public function create()
  {
    ...
    $max_facility_number = Facility::withTrashed()->where('year', now()->year)->max('facility_number');
    $branches = Branch::where('is_active', 1)
      ->whereHas('users', function ($q) {
        $q->where('users.id', auth()->user()->id);
      })->get();
    ...
    return view('content.facilities.create', [
  ...
  public function store(Request $request)
  {
    ...
    $validated = $request->validate(
      [
//        'facility_number' => 'required|unique:facilities|integer',
//        'facility_number' => ['required', 'integer',
//          Rule::unique(Facility::class)->where(function ($query) use ($request) {
...
//        'facility_number.integer' => 'يجب أن يكون حقل رقم التسهيل عددًا صحيحًا.',
    ...
    return redirect()->route('facilities.show', ['facility' => $facility->id])
  ...
  public function show(string $id)
  {
    ...
    $facilityBranch = Facility::with('branch')->findOrFail($id);
...
//      abort(403);
    ...
    return view('content.facilities.show', ['facility' => $facility]);
  }
...
//      abort(403);
    ...
    if ($facility->status_id == 1) {
      ...
      return view('content.facilities.edit', [
        'facility' => $facility,
        'branches' => $branches,
        'units' => $units,
        'currencies' => $currencies,
  ...
  public function exportFacility(string $id)
  {
    ...
    $facility = Facility::with('branch', 'currency', 'type', 'specialization', 'products')->findOrFail($id);

    if (!in_array($facility->branch->id, $userBranches)) {
      abort(403);
    }

    $fileName = Str::limit($facility->recipient, 30);
    $fileBranch = $facility->branch->name;
    $pdf = PDF::loadView('content.facilities.exportFacility', ['facility' => $facility]);
